...
export const SkillBoxFlip = ({ skill }: { skill: SkillCard }) => {
    return (
        <div className='row-span-2 group w-full h-full [perspective:1000px] drop-shadow-xl'>
            <div
            ...
            >
                <div
                    className={`absolute w-full h-full rounded-2xl ${skill.background}
                    flex justify-center items-center text-sm md:text-base font-bold md:text-xl`}
                >
                    <p className='px-5'>{skill.id}</p>
                </div>
                <div
                    className='absolute w-full h-full rounded-2xl bg-white [transform:rotateY(180deg)] [backface-visibility:hidden] 
                    flex flex-col justify-center items-center text-xs md:text-sm font-semibold'
                >
                    {skill.desc.map((val) => {
                        return <p className='w-full px-5 py-1'>{val}</p>;
                    })}
                </div>
            </div>
        </div>
