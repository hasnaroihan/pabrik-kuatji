...
export const AboutMeToggle = ({isToggleShown, handleClickToggle}: {isToggleShown: boolean, handleClickToggle: MouseEventHandler<HTMLButtonElement>}) => {
    return (
        <div className={`flex flex-col w-1/3 min-w-fit h-full min-h-full ml-5 md:ml-10 bg-white rounded-b-xl drop-shadow-xl
            justify-end items-center gap-2 transition transform duration-500 ${
            isToggleShown ? "ease-in translate-y-[70%] md:ease-in translate-y-[60%]" : "ease-out translate-y-0"}`}>
            <div className="flex flex-col w-fit text-accent justify-start items-start">
                <p className="text-lg md:text-xl font-mansalva">Pabrik Kuatji by</p>
                <p className="text-2xl md:text-4xl font-mansalva font-bold">Hasna Roihan</p>
            </div>
